...
$newClone.children("input").eq(0).attr("id",'componente'+newID).val('');
$newClone.children("input").eq(2).attr("id",'cantidad'+newID).val('0');
$newClone.children("input").eq(3).attr("id",'precio'+newID).val('0');
$newClone.children("input").eq(4).attr("id",'total'+newID).val('0');
...
function delRow() {
 ...
 //var componente =$(this).find("input[name^=componente").val();    
  
 //var componente = document.form_name.elements["componente[]"].val;  
//if(componente != ""){

  

if  (  confirm('ESTA SEGURO DE ELIMINAR COMPONENTE'))$(this).parent('div').remove() ; 
